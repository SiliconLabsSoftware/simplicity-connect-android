...
class DemoItemProvider {
    companion object {
        private fun getDataTest(deviceMacAddress: String): ArrayList<ItemTestCaseInfo> {
            return ArrayList<ItemTestCaseInfo>().apply {
                add(ItemTestCaseInfo(1, "Scan",
                    "Scanning for device \"$deviceMacAddress\".",
        ...
        fun createDataSiliconLabsTest(fwName: String, deviceMacAddress: String): SiliconLabsTestInfo {
            return SiliconLabsTestInfo(fwName, deviceMacAddress, getDataTest(deviceMacAddress))
        }
